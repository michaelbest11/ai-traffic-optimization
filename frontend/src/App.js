...
// ------------------------------------
const RouteRecommendation = ({ onRouteGenerated }) => {
  ...
  const handleOptimize = () => {
    ...
    const mockData = {
      ...
      ai_insights: "Route generated without API using local optimization model.",
    };
  ...
  return (
    <div className="bg-white p-4 rounded shadow mb-4">
      <h2 className="text-xl font-bold mb-3">🚦 Smart Route Optimization</h2>
      ...
      {routeData && (
        <div className="bg-gray-100 p-3 rounded border mt-4">
          ...
          <p><strong>Time:</strong> {routeData.estimated_time_minutes} mins</p>
          <p className="mt-2 bg-white p-2 rounded border">{routeData.ai_insights}</p>
        </div>
      )}
    </div>
  );
